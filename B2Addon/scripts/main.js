...
function processTntQueue() {
  ...
  for (let i = 0; i < amount; i++) {
    const pos = randomAround(target, 4, 7);
    try {
      target.runCommand(`summon tnt ${pos.x} ${pos.y} ${pos.z}`);
    } catch (error) {
...
world.afterEvents.scriptEventReceive.subscribe((event) => {
  ...
  if (event.id === ACTION_EVENT) { if (data.action === 'follow') { tntQueue += 5; showEvent('➕ SEGUIR', '+5 TNT — explota cerca de ti'); } if (data.action === 'share') { giveEnchantedApple(); showEvent('↗️ COMPARTIR', '+1 Manzana encantada'); } }
});
